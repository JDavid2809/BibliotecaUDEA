...
const Navigation: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAreas = async () => {
      try {
        ...
        const areasFormateadas: Carrera[] = response.data.map((area: { id: number; nombre: string; idArea?: number; nombreArea?: string }) => ({
          ...area,
      ...
      if (carrera && carrera.idArea) {
        try {
          const response = await axios.get(
            `http://localhost:4000/api/semestre/?fkIdArea=${carrera.idArea}`
          );
    
          setCarrera((prevCarrera) => ({
            ...prevCarrera!,
            semestres: response.data.map((semestre: { idSemestre: number; nombreSemestre: string; materias: { id: number; nombreMateria: string }[] }) => ({
  ...
  const agregarSemestre = async ({ nombreSemestre, fkIdArea }: { nombreSemestre: string; fkIdArea: number }) => {
    if (!carrera) return;
    try {
      const response = await axios.post("http://localhost:4000/api/semestre/", {
        nombreSemestre,
        fkIdArea,
      });
      ...
      setCarrera((prevCarrera) => {
        ...
        const nuevosSemestres = [...(prevCarrera.semestres || []), nuevoSemestre];
  ...
  <>
    ...
    />
      ...
      {modalAbierto === "carrera" && (
        <AgregaCarrera isOpen onClose={() => setModalAbierto(null)} onAdd={agregarCarrera} />
      )}
