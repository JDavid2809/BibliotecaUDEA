...
const EditarUsuario: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`http://localhost:4000/api/administrador/updateAdminByEmail/${email}`, {
        method: "PUT",
        headers: {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      try {
        setLoading(true)
        const response = await fetch(`http://localhost:4000/api/administrador/getAdminByEmail/${email}`)
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-2/3 bg-white rounded-lg shadow-md p-8">
          ...
          <div className="mb-6">
            ...
            {errors.nombre && <p className="text-red-500 text-sm mt-1 ml-7">{errors.nombre}</p>}
          </div>
          ...
          <div className="mb-6">
            ...
            {errors.telefono && <p className="text-red-500 text-sm mt-1 ml-7">{errors.telefono}</p>}
          </div>
