...
const TableDesign = () => {
  ...
  const fetchAdmins = async () => {
    try {
      const res = await axios.get("http://localhost:4000/api/administrador/getAllAdmins");
  ...
  const handleDeleteUser = async (email: string) => {
    ...
    try {
      await axios.delete(`http://localhost:4000/api/administrador/deleteAdminByEmail/${email}`);
      setAdmins((prev) => prev.filter((admin) => admin.correo !== email)); // Eliminar del estado local
      setFilteredAdmins((prev) => prev.filter((admin) => admin.correo !== email)); // Actualizar la lista filtrada
      ...
      {loading ? (
        ...
        <>
              ...
              <tbody>
                {filteredAdmins.map((persona, index) => (
                  <tr key={index} className="border-b hover:bg-gray-300">
                    ...
                    <td className="py-4 px-6 text-lg">{persona?.nombre || "No disponible"}</td>
                    <td className="py-4 px-6 text-lg">{persona?.telefono || "No disponible"}</td>
                    <td className="py-4 px-6 text-lg">{persona?.correo || "No disponible"}</td>
                    <td className="py-4 px-6">
          ...
          <div className="sm:hidden">
            {filteredAdmins.map((persona, index) => (
              <div key={index} className="mb-4 border bg-gray-200 p-4 rounded-md">
                <div className="flex items-center mb-4">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center">
                    <CircleUserRound size={60} />
                  </div>
                  <div className="ml-4">
                    <p className="text-lg font-semibold">{persona?.nombre || "No disponible"}</p>
